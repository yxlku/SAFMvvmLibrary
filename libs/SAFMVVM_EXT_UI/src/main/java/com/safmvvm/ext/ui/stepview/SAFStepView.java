package com.safmvvm.ext.ui.stepview;
...
public class SAFStepView extends View {
    ...
    public SAFStepView(Context context) {
        this(context, null);
    }

    public SAFStepView(Context context, AttributeSet attrs) {
        this(context, attrs, R.attr.sv_stepViewStyle);
    }

    public SAFStepView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void applyStyles(Context context, AttributeSet attrs, int defStyleAttr) {
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.StepView, defStyleAttr, 0);
        ...
        isDoneDuihao = ta.getBoolean(R.styleable.StepView_sv_done_duihao, true);
    ...
    private void drawStep(Canvas canvas, int step, int circleCenterX, int circleCenterY) {
        ...
        if (isSelected && !isDone) {
            ...
            if (isDoneDuihao) {
                ...
                paint.setColor(doneCircleColor);
                int radius = doneCircleRadius;
                if (state == ANIMATE_STEP_TRANSITION && (animationType == ANIMATION_CIRCLE || animationType == ANIMATION_ALL)
                        && nextAnimatedStep < currentStep) {
                    if (!nextStepCircleEnabled || nextStepCircleColor == 0) {
                        radius = (int) (doneCircleRadius - doneCircleRadius * animatedFraction);
                    } else {
                        radius = doneCircleRadius;
                    }
                    if (nextStepCircleEnabled && nextStepCircleColor != 0) {
                        paint.setColor(ColorUtils.blendARGB(
                                selectedCircleColor,
    ...
    public class State {
        ...
        @AnimationType
        private int animationType = SAFStepView.this.animationType;
        @ColorInt
        private int selectedCircleColor = SAFStepView.this.selectedCircleColor;
        @Dimension
        private int selectedCircleRadius = SAFStepView.this.selectedCircleRadius;
        @ColorInt
        private int selectedTextColor = SAFStepView.this.selectedTextColor;
        @ColorInt
        private int doneCircleColor = SAFStepView.this.doneCircleColor;
        @Dimension
        private int doneCircleRadius = SAFStepView.this.doneCircleRadius;
        @ColorInt
        private int doneTextColor = SAFStepView.this.doneTextColor;
        @ColorInt
        private int nextTextColor = SAFStepView.this.nextTextColor;
        @Dimension
        private int stepPadding = SAFStepView.this.stepPadding;
        @ColorInt
        private int nextStepLineColor = SAFStepView.this.nextStepLineColor;
        @ColorInt
        private int doneStepLineColor = SAFStepView.this.doneStepLineColor;
        @Dimension
        private int stepLineWidth = SAFStepView.this.stepLineWidth;
        @Dimension(unit = Dimension.SP)
        private float textSize = SAFStepView.this.textSize;
        @Dimension
        private int textPadding = SAFStepView.this.textPadding;
        @ColorInt
        private int selectedStepNumberColor = SAFStepView.this.selectedStepNumberColor;
        @Dimension(unit = Dimension.SP)
        private float stepNumberTextSize = SAFStepView.this.stepNumberTextSize;
        @ColorInt
        private int doneStepMarkColor = SAFStepView.this.doneStepMarkColor;
        private int animationDuration = SAFStepView.this.animationDuration;
        private boolean nextStepCircleEnabled = SAFStepView.this.nextStepCircleEnabled;
        @ColorInt
        private int nextStepCircleColor = SAFStepView.this.nextStepCircleColor;
        ...
        public void commit() {
            SAFStepView.this.animationType = animationType;
            SAFStepView.this.selectedTextColor = selectedTextColor;
            SAFStepView.this.selectedCircleRadius = selectedCircleRadius;
            SAFStepView.this.selectedCircleColor = selectedCircleColor;
            SAFStepView.this.doneCircleColor = doneCircleColor;
            SAFStepView.this.doneCircleRadius = doneCircleRadius;
            SAFStepView.this.doneTextColor = doneTextColor;
            SAFStepView.this.nextTextColor = nextTextColor;
            SAFStepView.this.stepPadding = stepPadding;
            SAFStepView.this.nextStepLineColor = nextStepLineColor;
            SAFStepView.this.doneStepLineColor = doneStepLineColor;
            SAFStepView.this.stepLineWidth = stepLineWidth;
            SAFStepView.this.textSize = textSize;
            SAFStepView.this.textPadding = textPadding;
            SAFStepView.this.selectedStepNumberColor = selectedStepNumberColor;
            SAFStepView.this.stepNumberTextSize = stepNumberTextSize;
            SAFStepView.this.doneStepMarkColor = doneStepMarkColor;
            SAFStepView.this.animationDuration = animationDuration;
            setTypeface(typeface);
            SAFStepView.this.nextStepCircleEnabled = nextStepCircleEnabled;
            SAFStepView.this.nextStepCircleColor = nextStepCircleColor;
            if (steps != null && !SAFStepView.this.steps.equals(steps)) {
                SAFStepView.this.setSteps(steps);
            } else if (stepsNumber != 0 && stepsNumber != SAFStepView.this.stepsNumber) {
                SAFStepView.this.setStepsNumber(stepsNumber);
            } else {
                SAFStepView.this.invalidate();
            }
        }
    }
}
