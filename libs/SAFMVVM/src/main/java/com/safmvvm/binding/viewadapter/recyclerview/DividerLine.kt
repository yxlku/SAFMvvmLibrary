...
class DividerLine(context: Context) : ItemDecoration() {
    ...
    private fun drawVertical(
        ...
        for (i in 0 until childCount) {
            ...
            val right =
                if (mDividerSize == 0) left + DensityUtil.dp2px(DEFAULT_DIVIDER_SIZE.toFloat()) else left + mDividerSize
    ...
    private fun drawHorizontal(
        ...
        for (i in 0 until childCount) {
            ...
            val bottom =
                if (mDividerSize == 0) top + DensityUtil.dp2px(DEFAULT_DIVIDER_SIZE.toFloat()) else top + mDividerSize
            mDividerDrawable!!.setBounds(left, top, right, bottom)
            //画上去
            mDividerDrawable.draw(c)
        }
