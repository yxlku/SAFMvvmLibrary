 ...
 */
public final class NewLocalMediaPageLoader {
    private static final String TAG = NewLocalMediaPageLoader.class.getSimpleName();
    ...
    public NewLocalMediaPageLoader(Context context) {
...
//    }
     ...
     * @param listener
     */
    public void loadAllMediaCount(int severalCount, OnQueryDataResultListener<LocalMediaFolder> listener) {
        PictureThreadUtils.executeByIo(new PictureThreadUtils.SimpleTask<List<LocalMediaFolder>>() {
            @Override
            public List<LocalMediaFolder> doInBackground() {
                Cursor data = mContext.getContentResolver().query(QUERY_URI,
                ...
                try {
                    if (data != null) {
                        int count = data.getCount();
                        int several;
                        if(severalCount <= count){
                            several = severalCount;
                        }else{
                            several = count;
                        }
                        int totalCount = 0;
    ...
    private static NewLocalMediaPageLoader instance;

    public static NewLocalMediaPageLoader getInstance(Context context) {
        if (instance == null) {
            synchronized (NewLocalMediaPageLoader.class) {
                if (instance == null) {
                    instance = new NewLocalMediaPageLoader(context.getApplicationContext());
                }
            }
        }
        return instance;
