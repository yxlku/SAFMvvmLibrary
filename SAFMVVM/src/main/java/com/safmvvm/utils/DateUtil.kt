...
fun Calendar.getTimeSpan(otherCalendar: Calendar = Calendar.getInstance(), unit: TimeUnit): Long =
    calculateTimeSpan(
        Math.abs(this.timeInMillis - otherCalendar.timeInMillis),
        unit
    )
...
fun Date.getTimeSpan(otherDate: Date = Date(), unit: TimeUnit = TimeUnit.DAYS): Long =
    calculateTimeSpan(
        Math.abs(this.time - otherDate.time),
        unit
    )
 ...
 * 其余显示，2017-09-01
 */
fun formatAgoStyleForWeibo(time: String, format: DateFormat = DefaultDateFormat.DEFAULT_FORMAT.get()!!): String = parseDateString2Mills(
    time,
    format
 ...
 * 其余显示，2017-09-01
 */
fun formatAgoStyleForWeChat(time: String, format: DateFormat = DefaultDateFormat.DEFAULT_FORMAT.get()!!): String = parseDateString2Mills(
    time,
    format
